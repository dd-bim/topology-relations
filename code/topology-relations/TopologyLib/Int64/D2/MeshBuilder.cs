...
namespace TopologyLib.Int64.D2
{
    public static class MeshBuilder
    {
        public static bool CreateFromGeometryOgc(in IReadOnlyList<IGeometryOgc2<decimal>?> geometries,
            out Mesh? mesh, out Dec2.Conversion64 conversion,
            out ImmutableDictionary<int, IGeometry> meshGeometries)
        {
            ...
            conversion = new Dec2.Conversion64(box);

            // Punkte in Ganzzahlen umwandeln
            var iPoints = new List<Vector>(points.Count);
            var lineStrings = new List<LineString>();
            var iGeometries = new List<IGeometryOgc2<long>?>(geometries.Count);
            foreach (var geometry in geometries)
            {
                switch (geometry)
                {
                    case Dec2.Vector v:
                        var tv = conversion.Convert(v);
                        iPoints.Add(tv);
                        iGeometries.Add(tv);
                        break;
                    case Dec2.LineString l:
        ...
        private class MeshEdge : SortedList<Fraction128, IIntegerPoint>
        {
            ...
            public SortedList<Fraction128, Vertex> CachedVertices;
            ...
            public MeshEdge(in Vertex orig, in Vertex dest, in bool isMeshEdge = false)
            {
                ...
                Add(Fraction128.Zero, orig.Point);
                Add(Fraction128.One, dest.Point);
                HalfEdges = new Dictionary<IIntegerPoint, HalfEdge> { { orig.Point, he! } };
                CachedVertices = new SortedList<Fraction128, Vertex>();
                IsUsed = !isMeshEdge;
            }

            public void Split(in Fraction128 pos, in Vertex vertex)
            {
        ...
        private class Builder
        {
            ...
            public void AddEdge(in Vertex orig, in Vertex dest)
            {
                ...
                for (; ; )
                {
                    ...
                    var lastDet = destVector.Det(curVector, (Vector)curHalfEdge.Right.Dest.Point);
                    foreach (var halfEdge in curHalfEdge.StarCcw())
                    {
                        var curDestVector = (Vector)halfEdge.Dest.Point;
                        var det = destVector.Det(curVector, curDestVector);
                        if (lastDet > 0 && det < 0)
                        {
                    ...
                    for (; ; )
                    {
                        ...
                        var det = destVector.Det(curVector, (Vector)opp.Point);
                        if (det == 0)
                        {
