...
{
    public readonly struct LineString : IReadOnlyList<Vector>, IGeometryOgc2<long>
    {
        ...
        private static Int128 GetArea2(bool isClosed, in ImmutableArray<Vector> vertices)
        {
            if (isClosed && vertices.Length > 3 
                && (ImmutableHashSet.CreateRange(vertices).Count + 1) == vertices.Length)
            {
                Int128 area = ((Int128)vertices[0].x * (vertices[1].y - vertices[^2].y));
                var prev = vertices[0];
                for (int i = vertices.Length - 2; i > 0; i--)
                {
                    area += ((Int128)vertices[i].x * (prev.y - vertices[i - 1].y));
                    prev = vertices[i];
                }
                return area;
            }
        ...
        internal LineString(in BBox bBox, in ImmutableArray<Vector> vertices, in bool isClosed, in Int128 area2)
        {
            BBox = bBox;
            Vertices = vertices;
            IsClosed = isClosed;
        ...
        public static bool TryParse(in string input, bool isLinearRing, out LineString lineString, out int end, char separator = ',', char vectorSeparator = ' ')
        {
            ...
            int start = Min(input.Length, input.IndexOf('(') + 1);
            int closedEnd = input.IndexOf(')', start);
