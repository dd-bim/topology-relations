...
{
    public readonly struct Vector : IEquatable<Vector>, IComparable<Vector>, IIntegerPoint, IGeometryOgc2<int>, IVector<int>
    {
            ...
            out ulong num1, out ulong num2, out ulong den)
        {
            ...
            long lnum1 = cd.Det(ac);
            long lnum2 = ab.Det(ac);
            long lden = cd.Det(ab);
            ...
            out ulong den)
        {
            ...
            long lnum1 = ab2.Det(aa);
            long lden = ab2.Det(ab1);
            // force positive denominator
            if (lden < 0)
            {
                lnum1 = -lnum1;
        ...
        {
            if (Intersect(a1, b1, a2, b2, out ulong num1, out ulong num2, out ulong den))
            {
        ...
        {
            if (Intersect(a1, b1, a2, b2, out ulong num1, out ulong den))
            {
                pos1 = new Numb.Fraction(num1, den);
                return true;
            }
        ...
        {
            if (PositionOf(a, b, point, out ulong num, out ulong den))
            {
        ...
        {
            long bx = (long)edgeB.x - edgeA.x;
            long tx = (long)x - edgeA.x;
            long by = (long)edgeB.y - edgeA.y;
            long ty = (long)y - edgeA.y;
            long det = (bx * ty) - (by * tx);
            long dotx = bx * tx;
            long doty = by * ty;
    ...
    public bool InCircle(in Vector a, in Vector b, in Vector c) // => inCircle(a - this, b - this, c - this);
        {
            ...
            long abdet = adx * bdy - bdx * ady;
            long bcdet = bdx * cdy - cdx * bdy;
            long cadet = cdx * ady - adx * cdy;
        ...
        public static bool TryParse(in string input, out Vector vector, char separator = ' ')
        {
            ...
            if (!string.IsNullOrEmpty(str))
            {
                ...
                if (split.Length >= 2
                             && int.TryParse(split[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out int x)
                             && int.TryParse(split[1], NumberStyles.Integer, CultureInfo.InvariantCulture, out int y))
                {
